...
if BATCH:
    checkpoint estimate_tx_abundance:
        ...
        run:
            ...
            LOG_ORFPRED.info(f"Estimated transcript abundance in {elapseTime:.2f} seconds")

    def get_RSEMIsoRes_batch(wildcards):
        checkpointOut = checkpoints.estimate_tx_abundance.get(**wildcards).output.complete
        batchFile = BATCH_UDT if TRIM else BATCH
        df = pd.read_csv(batchFile, sep="\t", header=None)
        return [join(TRANSEVID_DIR, "trinityToolkitOut", dir, "RSEM.isoforms.results") for dir in df.iloc[:, 1].tolist()]
    ...
    checkpoint build_expr_matrics:
        ...
        output:
            RSEMIsoTPM = join(TRANSEVID_DIR, "trinityToolkitOut", "RSEM.isoform.TMM.EXPR.matrix")
        log:
        ...
        run:
            ...
            endTime = time()
            elapseTime = endTime - startTime
            LOG_ORFPRED.info(f"Built transcript and gene expression matrics in {elapseTime:.2f} seconds")

    def get_RSEMIsoTPM_batch(wildcards):
        checkpointOut = checkpoints.build_expr_matrics.get(**wildcards).output.RSEMIsoTPM
    ...
    rule filter_based_on_exprLevel:
        ...
        input:
            ...
            transcriptPep = join(ORFPREDICT_DIR, "transDecoder", "transcript.flt1.fa.transdecoder.pep")
        output:
            transcriptNoBlast2TPM1fa = temp(join(TRANSEVID_DIR, "transcript.noBlast2.tpm1.fasta")),
            transcriptNoBlast2TPM1ID = temp(join(TRANSEVID_DIR, "transcript.noBlast2.tpm1.id.txt")),
            transcriptNoBlast2NoTPM1ID = temp(join(TRANSEVID_DIR, "transcript.noBlast2.notpm1.id.txt")),
            transcriptFltFinalID = temp(join(TRANSEVID_DIR, "transcriptFltFinal.id.txt")),
            transcriptFlt1Pep = temp(join(ORFPREDICT_DIR, "transDecoder", "transcript.flt1.pep")),
        ...
        run:
            ...
            endTime = time()
            elapseTime = endTime - startTime
            LOG_EXPRFLT.info(f"Filtered transcripts based on expression values in {elapseTime:.2f} seconds")
else:
    if PAIREDEND:
        checkpoint estimate_tx_abundance:
            """
            ...
            input:
                ...
                transcript = join(TRANSEVID_DIR, "transcript.noBlast2.fasta")
            output:
                RSEMIsoRes = join(TRANSEVID_DIR, "trinityToolkitOut", "RSEM.isoforms.results")
            log:
                join(OPTIM_LOG_DIR, "exprFlt-estimate_tx_abundance.log")
            params:
                trinityToolkitOutdir = join(TRANSEVID_DIR, "trinityToolkitOut"),
        ...
        def get_RSEMIsoRes(wildcards):
            checkpointOut = checkpoints.estimate_tx_abundance.get(**wildcards).output.RSEMIsoRes
            return checkpointOut
        ...
        checkpoint build_expr_matrics:
            ...
            output:
                RSEMIsoTPM = join(TRANSEVID_DIR, "trinityToolkitOut", "RSEM.isoform.TPM.not_cross_norm")
            log:
                join(OPTIM_LOG_DIR, "exprFlt-build_expr_matrics.log")
            params:
                RSEMprefix = join(TRANSEVID_DIR, "trinityToolkitOut", "RSEM")
