...
if TRIM:
    if BATCH:
        if PAIREDEND:
            rule fastp_cleanFQ:
                ...
                run:
                    ...
                    LOG_FQTRIM.info(f"Preprocessed input FASTQ file of {wildcards.sample} in {elapseTime:.2f} seconds")
        else:
            rule fastp_cleanFQ:
                """ 
                Preprocess input FASTQ file
        ...
        if df.shape[1] == 3:
            ...
            df["fq"] = df["fq"].apply(lambda path: cf.process_path(path, PREPROC_DIR))
        elif df.shape[1] == 4:
            df.columns = ["cond", "rep", "fq1", "fq2"]
            df["fq1"] = df["fq1"].apply(lambda path: cf.process_path(path, PREPROC_DIR))
            df["fq2"] = df["fq2"].apply(lambda path: cf.process_path(path, PREPROC_DIR))
        else:
        ...
        if PAIREDEND:
            rule fastp_cleanFQ:
                ...
                run:
                    ...
                    endTime = time()
                    elapseTime = endTime - startTime
                    LOG_FQTRIM.info(f"Preprocessed input FASTQ file in {elapseTime:.2f} seconds")
        else:
            rule fastp_cleanFQ:
                """ 
                Preprocess input FASTQ file
        ...
        if PAIREDEND:
            rule copy_fastq:
                ...
                run:
                    ...
                    endTime = time()
                    elapseTime = endTime - startTime
                    LOG_FQTRIM.info(f"Copied FASTQ file(s) of {wildcards.sample} to objective directory {elapseTime:.2f} seconds")
        else:
            rule copy_fastq:
                """
                Copy FASTQ file(s) to objective directory
        ...
        if PAIREDEND:
            rule copy_fastq:
                ...
                run:
                    ...
                    endTime = time()
                    elapseTime = endTime - startTime
                    LOG_FQTRIM.info(f"Copied FASTQ file(s) to objective directory {elapseTime:.2f} seconds")
        else:
            rule copy_fastq:
                """
                Copy FASTQ file(s) to objective directory
