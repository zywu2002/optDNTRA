...
rule filter_based_on_database:
    ...
    output:
        ...
        transcriptFltNoBlast2ID=temp(
            join(TRANSEVID_DIR, "transcriptFlt.noBlast2.id.txt")
        ),
    log:
    ...
    run:
        ...
        LOG_DBFLT.info(
            f"Performed BLAST search against Swiss-Prot database and filtered transcripts in {elapseTime:.2f} seconds."
        )
